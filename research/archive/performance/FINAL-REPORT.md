...
>
> 本调研基于 **1.0.18 之前**的代码基线，2026-06 后续重构后部分"✅ 已修复"标记**已不准确**：
>
> | 调研结论                                        | 2026-06 实际状态                                                                                                                              |
> | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
> | #4 WindowFocusListener 无限累积 **"✅ 已修复"** | **整个 listener 机制已删除**（连同 `requestBrowserFocus()`）— jcef-focus-ime 调研方向被实测无效，相关代码不再存在。**严格说不是"修"，是"删"** |
> | jcef-focus-ime 关联                             | `research/jcef-focus-ime/` 整个目录已删除                                                                                                     |
>
> 当前代码基线不包含被删的代码。本报告保留为**历史调研记录**，不应再作为"待办"或"现状"参考。**"已修复"标记**仅指调研时点（1.0.18 之前）的代码状态，**不代表当前状态**。
...
**关联**：无（2026-06 清理：原"与 jcef-focus-ime 密切相关"的关联已删除，jcef-focus-ime 调研整体废弃）。
...
| 目录                                | 内容                                    |
| ----------------------------------- | --------------------------------------- |
| `research/jcef-focus-ime/`          | **已删除**（2026-06 整体废弃）          |
