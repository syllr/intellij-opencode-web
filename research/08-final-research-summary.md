...
| 05   | [OpenCode V2 API 详解](./05-opencode-v2-api-details.md)                           | V2 API 端点、消息结构、认证机制            |
| 06   | [JCEF 修复测试策略](./06-testing-strategies.md)                                   | 单元测试、集成测试、手动测试、性能测试     |
| 07   | [CefFocusHandler.FocusSource 枚举详解](./07-ceffocushandler-focus-source-enum.md) | 枚举值、触发场景、平台特定行为             |
| 08   | [最终调研总结](./08-final-research-summary.md)                                    | 本文档                                     |
...
| 枚举值 | 说明 | 触发场景 |
|--------|------|----------|
| `FOCUS_SOURCE_NAVIGATION` | 键盘导航触发 | Tab 键、方向键 |
| `FOCUS_SOURCE_SYSTEM` | 系统事件触发 | 窗口激活、鼠标点击 |

**平台特定行为**:

- **Windows**: 导航来源时可能禁用浏览器焦点
- **macOS**: 通常允许焦点正常传递
- **Linux**: 可能需要手动调用 `requestFocus()`
...
| JCEF FocusSource API         | http://www.xuanyimao.com/doc/jcef/202009/org/cef/handler/CefFocusHandler.FocusSource.html                      |
...
1. **深入分析了 JCEF 失焦/IME 问题的根因** - 确认缺少 `CefFocusHandler` 和 `CefCompositionHandler` 实现
2. **调研了 IDEA 插件与 opencode 的联动机会** - 识别了 8 个高价值联动点
3. **详细分析了 opencode 后端 API** - 包括 V1 和 V2 API 的完整规范
4. **提供了具体的技术实现方案** - 包括代码示例和测试策略
5. **分析了 CefFocusHandler.FocusSource 枚举** - 提供了完整的枚举值和处理建议
