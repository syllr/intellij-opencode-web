...
| 14   | [最终总结](./14-final-research-summary.md)                                        | 本文档                                     | ✅ 完成 |
...
- `PsiFile` - 表示整个文件
- `PsiElement` - 表示代码元素
- `PsiDocumentManager` - 管理 PSI 和 Document 的映射
- `PsiManager` - 管理 PSI 文件
- `PsiReference` - 表示代码引用
...
- `GitRepositoryManager` - 管理 Git 仓库
- `GitRepository` - 表示 Git 仓库
- `Git` - 执行 Git 命令
- `GitChangeListManager` - 管理变更列表
...
- `NotificationGroupManager` - 管理通知组
- `Notification` - 通知实例
- `ProgressManager` - 管理进度指示器
- `Backgroundable` - 后台任务
