...
- 焦点恢复:JCEF 焦点卡死时由用户**手动**通过右键菜单 Reset 触发 `resetToolWindow`(`LinkContextMenuHandler`);无自动焦点恢复监听器
...
- **JCEF 焦点问题**（"OpenCodeWeb 面板键盘不响应 / 输入法卡顿"）唯一的实测有效恢复手段是 `MyToolWindowFactory.resetToolWindow(project)`（hide + activate ToolWindow）。`requestBrowserFocus()`（仅调 Swing `requestFocus()`）和 `FocusAdapter` 方向都**实测无效**，**不要**再尝试。**自动焦点恢复 listener 已全部移除**（`ToolWindowManagerListener` 位于 `com.intellij.openapi.wm.ex` 被 Marketplace Validator 标 internal API；`FocusAdapter.focusGained` 实测无效,未走 hide+activate）,当前**仅**通过右键菜单 Reset 手动触发 `resetToolWindow`。如未来需恢复自动焦点监听,需同时避开 internal API 警告和"未走 hide+activate"的无效恢复路径,SPEC.md / DESIGN.md 无现成方案可参考
