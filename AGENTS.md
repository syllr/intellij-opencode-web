...
- 焦点恢复：用户切到 OpenCodeWeb tab 时通过 `ToolWindow.component.addFocusListener(FocusAdapter.focusGained)` 自动调用 `resetToolWindow`（1.5s `AtomicLong` 防抖）。1.0.25 前曾用 `ToolWindowManagerListener.stateChanged`（位于 `com.intellij.openapi.wm.ex` 包），被 JetBrains Marketplace Validator 标 internal API 后替换为完全公开 API
...
- **JCEF 焦点问题**（"OpenCodeWeb 面板键盘不响应 / 输入法卡顿"）唯一**实测有效**的恢复手段是 `MyToolWindowFactory.resetToolWindow(project)`（hide + activate ToolWindow）。**已实测无效的方向**：(1) `requestBrowserFocus()` 仅调 Swing `requestFocus()`（无 hide+activate 重置）；(2) 早期 FocusAdapter 监听到焦点变化后直接调 `requestFocus()` 类恢复动作（未走 resetToolWindow）。**当前生效方案**：`ToolWindow.component.addFocusListener(FocusAdapter.focusGained)` → `resetToolWindow(project)`，1.5s `AtomicLong` 防抖（防 reset 内部 activate 循环 + 内部 input 切换误触），已实测有效。**触发器选 FocusAdapter 而非 ToolWindowManagerListener 的原因**：后者位于 `com.intellij.openapi.wm.ex` 包，被 Marketplace Validator 标 internal API（见 1.0.25 publish 报告）。`resetToolWindow` 本身保留供任何触发路径调用，是 JCEF 焦点恢复的唯一不变量
