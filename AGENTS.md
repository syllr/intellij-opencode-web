...
| `utils/`                       | HTTP API、IdeaVim 集成、通知路由/服务（M3-T4 no-op）、SSE consumer 工厂、OpenCodeConfig（7 文件） |
...
| **通知(M3-T4 后 IDE 端 no-op)**                |                                                                                   |
| 通知发送(`send()` 是 no-op 桩)                 | `utils/OpenCodeNotificationService.kt`(纯函数 helper + 5 单测保留)                |
...
- ~~`CleanBrowserLauncher` 用 `--user-data-dir=/tmp/...` 隔离 profile~~ → 整删（v2.0.0）,违反用户硬约束"浏览器不能用 --user-data-dir 隔离"（实测丢 localStorage 缓存导致项目选不中）;`OpenCodeBrowserLauncher` 复用日常 profile
- ~~in-IDE 通知 UI（`OpenCodeNotificationService` 双模式 BALLOON + SystemNotifications）~~ → 整删（v2.0.0 M3-T4）,`send()` 改 no-op 桩,Edge --app 模式由 OpenCode Web UI 自身接管
