...
| 服务器进程启停(4 阶段关闭)+ port 冲突检测(M2-T1)   | `toolWindow/OpenCodeServerManager.kt`(health gate 只检测不升级,冲突时 onFailed)                                 |
| 进程启动(cwd = project.basePath) + health gate     | `toolWindow/ServerProcessLauncher.kt`(M0-T1;`startServerSingleflight` 端口固定 12396)                           |
| Edge --app 启动器(项目隔离 profile)                | `toolWindow/OpenCodeBrowserLauncher.kt`(M1-T2;`--user-data-dir=~/.config/opencode-web-ui/edge-profiles/$hash/`) |
| Edge ext 资源写盘(manifest + content + background) | `toolWindow/EdgeBootstrapExtension.kt`(background.js 跑 `chrome.contentSettings.notifications.set` 预置 allow)  |
...
- 端口固定 **12396**(非标准,避免与 4096 冲突;**不**升级 — Edge extension `background.js` / `manifest.json` 硬编码 `localhost:12396/*`,`MyToolWindow.buildOpenUrl()` 同样硬编码 12396,升级会引入新失败模式。详见 `DESIGN.md §1.2` 决策表 / `SPEC §6.2`)
...
- ~~多端口(按 IDE 类型分配 12396-12412)~~ → 全部用 12396 + **端口固定不升级**(commit `327c6c2` 引入 profile 隔离后,Edge ext 端口硬编码;升级会引入 notification 预置 allow 错位)。多 server 并发反而更卡(实测确认)
