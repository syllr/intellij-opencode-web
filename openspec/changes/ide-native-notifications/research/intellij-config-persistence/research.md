# IntelliJ 配置持久化机制调研(代码同步版)

> **状态说明**:本文件是 2026-05-15 调研结果。原 plan 设计"28 个 key-value(11 事件开关 + 11 消息模板 + 4 通用 + 2 预留)",实际代码**只实施 4 个通用 setting**。本文档保留调研结论,标注实际落地。
...
### PropertiesComponent Keys 实际清单

`OpenCodeConfig.kt`(25 行)实际只暴露 4 个 setting:

| Key                                  | 类型    | 默认值 | 实际定义                  |
| ------------------------------------ | ------- | ------ | ------------------------- |
| `opencode.settings.notification`     | Boolean | `true` | `var notificationEnabled` |
| `opencode.settings.showProjectName`  | Boolean | `true` | `var showProjectName`     |
| `opencode.settings.showSessionTitle` | Boolean | `true` | `var showSessionTitle`    |
| `opencode.settings.minDuration`      | Int     | `0`    | `var minDuration`         |

### 未实施的 Keys(原 plan 设计但未落地)

| Key 类型                                        | 数量        | 状态          |
| ----------------------------------------------- | ----------- | ------------- |
| `opencode.event.{type}.enabled` (11 个事件开关) | 0 / 11 实施 | ❌ 全部未实施 |
| `opencode.message.{type}` (11 个消息模板)       | 0 / 11 实施 | ❌ 全部未实施 |

**实际行为**:

- 哪些事件触发通知 → 硬编码在 `OpenCodeSSEConsumer.onMessage()` 的 `when (type)` 分支
- 通知模板内容 → 硬编码在 `OpenCodeNotificationService.formatMessage()` 的 `when (eventType)` 分支
- 无 Settings UI 入口,用户**无法调整任何 setting**(只能手动改 workspace.xml)
