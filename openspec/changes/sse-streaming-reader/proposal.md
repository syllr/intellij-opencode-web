...
- **`server.heartbeat`**（10s 一次 keepalive）、`server.instance.disposed`（生命周期）
- **`server.connected`**——一次性事件，无用途
- **`session.error`**——error 不算 block
- **`session.updated`** / **`session.compacted`** / **`session.deleted`**——server 端状态变更，plugin 不关心
...
- **`message.updated`**（user_message 通知已砍 + state machine 重置信号已用不上）
...
- **`when (eventType)` 砍 4 个 case**（`server.connected` / `session.error` / `session.next.tool.called` / `message.updated`）共 -25 行
...
- `sse-stream-filter`: SSE 事件白名单解析能力。在 LaunchDarkly `streamEventData` 流式模式下，从 SSE message 的 Reader 中提前读取 `type` 字段（payload 直接为 root，无外层包装），与 `ALLOW_PARSE_EVENT_TYPES` 白名单（plugin 实际需要的 9 个 wire-level eventType，4 个通知 + 5 个业务）对比：**在白名单内**才做完整 Gson Map 解析进入 `when (eventType)` 分支，**不在白名单内**则直接 `close()` Reader 并 return，不构造完整 String、不调 Gson。**白名单外事件**包括 `message.part.delta`（100+ Hz 高频 token 增量）、`server.heartbeat`（10s keepalive）、`session.next.*` 子事件（25+ 个）等——全部跳过完整解析，节省 plugin 进程临时对象分配（理论估算 50-60 KB/s → ~5-10 KB/s，节省 80-90%）。
...
- **性能影响**（**以下为理论估算，待 spec 阶段 benchmark 验证**）：
  ...
  - **白名单比黑名单多省的部分**：25+ 个 plugin 不关心的噪音事件（`server.heartbeat` / `session.next.*` 子事件 / `project.updated` 等）**全部**跳过完整 Gson 解析，**而黑名单方案只能跳过 5 个 type + 2 个泛匹配**——白名单多覆盖 20+ 个事件
